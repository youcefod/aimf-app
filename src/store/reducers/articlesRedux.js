...
const SAVE_ARTICLES_REQUEST = "SAVE_ARTICLES_REQUEST";
const POST_ARTICLES_SUCCESS = "POST_ARTICLES_SUCCESS";
const POST_ARTICLES_ERROR = "POST_ARTICLES_ERROR";

const PUT_ARTICLES_SUCCESS = "PUT_ARTICLES_SUCCESS";
const PUT_ARTICLES_ERROR = "PUT_ARTICLES_ERROR";
...
const putArticleSuccess = (data) => {
  return {
    type: PUT_ARTICLES_SUCCESS,
    payload: data,
  };
};

const putArticleError = () => {
  return {
    type: PUT_ARTICLES_ERROR,
  };
};
...
export const createPost = (data) => {
  return (dispatch) => {
    ...
    getAxiosInstance()
      ...
      .then(function (response) {
        if (response.data.data.status === VALID_ARTICLE_STATUS) {
...
export const updatePost = (id, data) => {
  return (dispatch) => {
    dispatch(saveArticleRequest());
    getAxiosInstance()
      .put(`${GET_ARTICLES_URI}/${id}`, data)
      .then(function (response) {
        if (response.data.data.status === VALID_ARTICLE_STATUS) {
          dispatch(getArticles([], 1, true));
        }
        dispatch(putArticleSuccess(response.data.data));
      })
      .catch(function (error) {
        dispatch(
          batchActions(
            [dispatchError(error), putArticleError()],
            BATCH_ARTICLES_ERROR
          )
        );
      });
