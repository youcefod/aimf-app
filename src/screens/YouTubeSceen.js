    ...
    return (
      <ScrollView style={styles.container}>
        <YouTube
          ...
          apiKey="AIzaSyCI43XhQM9QSwKpYjSl9ki6CA2ikfWo1LU"
          // Un-comment one of videoId / videoIds / playlist.
          // You can also edit these props while Hot-Loading in development mode to see how
          // it affects the loaded native module
          videoId="mddmc_a9Rlw"
          // videoIds={['uMK0prafzw0', 'qzYgSecGQww', 'XXlZfc1TrD0', 'czcjU1w-c6k']}
          // playlistId="PLXGt1ohn-s4P_bH0Y1BL9E6bBLNnAp0l0"
        ...
        <View style={styles.buttonGroup}>
          <Button
            ...
            onPress={() => {
              if (this.youTubeRef.current) {
                this.youTubeRef.current.previousVideo();
              }
            }}
          />
          <Text> </Text>
          <Button
            ...
            onPress={() => {
              if (this.youTubeRef.current) {
                this.youTubeRef.current.nextVideo();
              }
            }}
          />
        </View>
        ...
        <View style={styles.buttonGroup}>
          <Button
            ...
            onPress={() => {
              if (this.youTubeRef.current) {
                this.youTubeRef.current.seekTo(15);
              }
            }}
          />
          <Text> </Text>
          <Button
            ...
            onPress={() => {
              if (this.youTubeRef.current) {
                this.youTubeRef.current.seekTo(2 * 60);
              }
            }}
          />
          <Text> </Text>
          <Button
            ...
            onPress={() => {
              if (this.youTubeRef.current) {
                this.youTubeRef.current.seekTo(15 * 60);
              }
            }}
          />
        </View>
        ...
        {this.youTubeRef.current &&
          this.youTubeRef.current.props.videoIds &&
          Array.isArray(this.youTubeRef.current.props.videoIds) && (
            <View style={styles.buttonGroup}>
              {this.youTubeRef.current.props.videoIds.map((videoId, i) => (
                <React.Fragment key={i}>
                  <Button
                    title={`Video ${i}`}
                    onPress={() => {
                      if (this.youTubeRef.current) {
                        this.youTubeRef.current.playVideoAt(i);
                      }
                    }}
                  />
                  <Text> </Text>
        ...
        {Platform.OS === "ios" && (
          <View style={styles.buttonGroup}>
            <Button
              ...
              onPress={() => {
                if (this.youTubeRef.current) {
                  this.youTubeRef.current.reloadIframe();
                }
              }}
            />
          </View>
